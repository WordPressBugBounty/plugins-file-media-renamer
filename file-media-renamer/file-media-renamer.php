    ...
    */
	...
	public function add_filename_column_content($column_name, $post_id) {
		...
		if ($column_name == 'filename') {
            ...
            echo '<input required class="FileMediaRenamer-filename" type="text" id="attachments-'.$post_id.'-filename" name="attachments['.$post_id.'][filename]" value="'.($filename ? $filename : '').'" /><span class="indicator"></span><span class="ext" style="font-weight:500; padding-left:5px;">.'.$ext.'</span>' ;
		}
                    ...
                    (function(w, d){
                        w.onload= function(){
                            if (wp.media) {
                                    ...
                                    var MediaRenderModel = wp.media.model.Attachment.extend( {
                                            saveCompat: function( t, s ) {
                                                var FileMediaRenamer_filename = d.querySelectorAll("input.FileMediaRenamer-filename");
                                                if(FileMediaRenamer_filename){
                                                            for(let i =0; i < FileMediaRenamer_filename.length; i++){
                                                                    input_filter(FileMediaRenamer_filename[i]);
                                                                    FileMediaRenamer_filename[i].addEventListener('blur', function(e){
                                                                    if(d.querySelector('input.FileMediaRenamer-filename')){
                                                                        $new_name =  d.querySelector('input.FileMediaRenamer-filename').value;

                                                                        if(d.querySelector('input.attachment-details-copy-link')){
                                                                            $filename_element = d.querySelector('input.attachment-details-copy-link');

                                                                            $old_filepath = $filename_element.value;
                                                                            $old_filename = $old_filepath.replace(/^.*[\\\/]/, ''); 
                                                                            $ext = $old_filename.split('.').pop();
                                                                            $new_filename = $new_name+'.'+$ext;

                                                                            if(d.querySelector('div.attachment-info div.details div.filename')){
                                                                                if(d.querySelector('div.attachment-info div.details div.filename > strong')){
                                                                                    d.querySelector('div.attachment-info div.details div.filename').innerHTML='<strong>Filename: </strong>'+$new_filename;
                                                                                }else{
                                                                                    d.querySelector('div.attachment-info div.details div.filename').innerHTML=$new_filename;
                                                                                }                                                                                
                                                                            }
                                                                            $filename_element.setAttribute('value',$old_filepath.replace($old_filename, $new_filename));
                                                                        }
                                                                    }
                                                                });
                                                            }
                                    ...
                                    wp.media.model.Attachment.prototype = MediaRenderModel.prototype;
                                ...
                                function FileMediaRenamer_submit_bulk_update(){
                                    ...
                                    if(FileMediaRenamer_fieldnames.length < 1){
                                        return;
                                    }
                                        ...
                                        var filename_preview = FileMediaRenamer_fieldnames[i].closest("tr").querySelector("p.filename");
                                        var incoming_filename = FileMediaRenamer_fieldnames[i].closest("tr").querySelector(".FileMediaRenamer-filename").value + FileMediaRenamer_fieldnames[i].closest("tr").querySelector("td.filename > span.ext").innerText;
                                        ...
                                        ajax_request(json2_payload({
                                                ...
                                                "action":"fmrequest"
                                            }), f_indicator, p_indicator, filename_preview, incoming_filename);
                                    }
                                ...
                                function ajax_request(json_data, f_indicator=null, p_indicator=null, filename_preview=null, filename_value=null){
                                    ...
                                    xhttp.onreadystatechange = function() {
                                         if (this.readyState == 4 && this.status == 200) {
                                            ...
                                            filename_preview.innerHTML= "<span class='screen-reader-text'>File name: </span>"+filename_value;
